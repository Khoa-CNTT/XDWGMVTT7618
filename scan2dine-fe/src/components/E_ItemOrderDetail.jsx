...
export const E_ItemOrderDetail = ({
    ...
    const handleDeleteItem = async (e) => {
        ...
        if (onDeleteItem) {
            setIsLoading(true);
            try {
                await onDeleteItem(item.id); // Gọi onDeleteItem để cập nhật state ở component cha
                    ...
                    <div className="flex justify-between items-center">
                        <div className="flex items-center">
                            <button
                                ...
                                className={`w-8 h-8 flex items-center justify-center rounded-full transition-colors ${isLoading || item.quantity <= 1 || (item.status !== '1' && item.status !== '2')
                            ...
                            >
                                <FaMinus
                                    className={`${isLoading || item.quantity <= 1 || (item.status !== '1' && item.status !== '2')
                                        ? 'text-gray-400'
                                        : 'text-gray-600'
                                        }`}
                                    size={12}
                            ...
                            <button
                                ...
                                className={`w-8 h-8 flex items-center justify-center rounded-full transition-colors ${isLoading || (item.status !== '1' && item.status !== '2')
                                    ? 'bg-gray-100 cursor-not-allowed'
                                    : 'bg-gray-200 hover:bg-red-500 hover:text-white'
                                    }`}
                                disabled={isLoading || (item.status !== '1' && item.status !== '2')}
                                title={item.quantity >= (item.maxQuantity || 100) ? 'Đã đạt số lượng tối đa' : ''}
                            >
                                <FaPlus
                                    className={`${isLoading || (item.status !== '1' && item.status !== '2') ? 'text-gray-400' : 'text-gray-600'
                                        }`}
                                    size={12}
                                />
                            </button>
                        ...
                        <button
                            ...
                            className={`p-2 rounded-full transition-colors ${isLoading || (item.status !== '1' && item.status !== '2')
