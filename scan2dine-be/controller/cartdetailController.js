...
const { increaseCartQuantity, decreaseCartQuantity, calculateCartdetail } = require('../service/cartdetailService');
...
const cartdetailController = {
    ...
    getCartdetail: async (req, res) => {
        try {
            const cartdetail = await CartDetail.find().populate({
                path: "products",
            ...
            const result = await Promise.all(cartdetail.map(async (item) => {
                const totalInfo = await calculateCartdetail(item._id);
                result.push(
                    {
